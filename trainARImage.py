...
def trainARImage(train_dataset_path, val_dataset_path, verbose=False, data_npy_exists = False):
    ...
    for epoch in range(numEpochs):
        ...
        with torch.no_grad():
            ...
            val_loss = criterion(output, val_imgs)


        # show the plots
        if epoch != 0:            
            axes[0].plot([int(epoch)-1, int(epoch)], [prevtrain_loss.cpu().detach().numpy(), train_loss.cpu().detach().numpy()/(i+1)], marker='o', color="blue", label="train")
            axes[1].plot([int(epoch)-1, int(epoch)], [prevval_loss.cpu().detach().numpy(), val_loss.cpu().detach().numpy()], marker='o', color="red", label="validation")
